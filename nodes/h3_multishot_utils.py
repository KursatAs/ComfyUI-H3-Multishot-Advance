# -*- coding: utf-8 -*-
"""H3 multishot utilities - single-script prompting for the
...
def _encode_ref_audio_compat(mmh3, audio_vae, audio):
    ...
    KursatAs 2026-08-18 05:33: prefer ComfyUI's current module-level helper,
    then the old class staticmethod, then H3's local compatibility fallback.

    ComfyUI master moved _encode_ref_audio from MiniMaxH3ReferenceToVideo to
    nodes_minimax_h3. Resolve the native helper at call time so both layouts
    keep working, and only mirror the behavior locally when neither exists.
    """
...
class H3MultishotMemorySampler:
    ...
    def run(self, model, clip, video_vae, audio_vae, script, shot_count, width,
        ...
        def _has_batch(_x):
            try:
                return _x is not None and int(_x.shape[0]) > 0
            except Exception:
                return _x is not None

        # KursatAs 2026-08-17 09:18: checkpoint warnings must follow real
        # ref-row sources, not only the memory_frames widget.
        _ref_rows_expected = (
            (n > 1 and cap > 0)
            ...
            if _blockers:
                ...
                try:
                    ...
                    _tdir = os.path.join(_fp.get_output_directory(), "video",
                                         "H3CHAIN_STREAM",
                                         "tmp_%d" % os.getpid())
                    _stream_assembler = ShotStreamAssembler(_tdir, fps=24)
        ...
        if upscale_model_name not in ("(none)", "", None):
            ...
            if _ram and _tot > _ram * 0.8:
                _limit = max(
                    1.0,
                    (_ram * 0.7 / (frames_per_shot * max(1, len(shots))
                                   * width * height * 3 * 2 / 2**30)) ** 0.5)
                _msg = ("that will not fit. Frames are held in system RAM "
                        "until the master is joined, so this dies at the join "
        ...
        if (chain_gain_control == "flatten_pin"
            ...
            # KursatAs 2026-08-17 20:48: flatten_pin is useful for A/B tests,
            # but it touches the raw context_pin latent before sampling and
            # can add to the same VRAM peak we are trying to reduce.
            _msg = ("chain_gain_control=flatten_pin is experimental and can "
                    "raise VRAM because it high-pass filters the raw "
        ...
        if bank_pinned == 0 and n > 4:
            _msg = ("bank_pinned=0 on a %d-shot chain. With no pinned slot "
                    "the conditioning is pure recency - each shot hears only "
                    "the one before it - and audio COLLAPSES: measured 84-92%% "
                    "loss of 4-10 kHz energy by shot 8 (five-arm A/B, "
        ...
        _cache_mode = str(shot_cache or "off").strip().lower()
        if _cache_mode == "read_write":
            _cache_mode = "use_cache"
        elif _cache_mode == "clear":
            _cache_mode = "rebuild_cache"
        _cache_enabled = _cache_mode in ("use_cache", "rebuild_cache")
        _cache_dir = None
        _cache_base_key = None
        _cache_start = 0

        def _cache_prefix_key(idx):
            return _h3_cache_key({"base": _cache_base_key,
                                  "prompts": shots[:idx]})

        def _cache_path(idx):
            import os
            key = _cache_prefix_key(idx)
            return os.path.join(_cache_dir, "prefix_%02d_%s.pt"
                                % (idx, key[:16]))
        ...
        def _cache_state():
            return {
                ...
                "lat_v_parts": _lat_v_parts,
                "lat_a_parts": _lat_a_parts,
                "sr": sr,
                ...
                "ho_v": _ho_v,
                "ho_a": _ho_a,
                "ho_taper_src": _ho_taper_src,
                "ho_guard": _ho_guard,
                "ho_wav_tail": _ho_wav_tail,
                ...
                "pin_sig0": _pin_sig0,
                "pin_hf0": _pin_hf0,
                "cg_last_raw": _cg_last_raw,
                "cp_trim": _cp_trim,
            }

        _cache_write_enabled = False

        def _save_cache_prefix(idx):
            nonlocal _cache_write_enabled
            if not _cache_write_enabled:
                return
            import os
            import torch as _torch
            path = _cache_path(idx)
            tmp = path + ".tmp"
            blob = {
                "version": _H3_SHOT_CACHE_VERSION,
                "base_key": _cache_base_key,
                "prefix_key": _cache_prefix_key(idx),
                "shot_index": int(idx),
                "state": _h3_cache_pack(_cache_state()),
            }
            try:
                ...
                print("[H3Memory] shot_cache: stored prefix %d/%d -> %s"
                      % (idx, n, os.path.basename(path)), flush=True)
            except Exception as _e:
                _cache_write_enabled = False
                try:
            ...
            if _stream_assembler is not None:
                ...
                try:
                    ...
                    _base_obj = {
                        ...
                        # KursatAs 2026-08-18 07:49: Keep total shot_count
                        # out of the base key so a 2-shot cache can resume
                        # when the user extends the same project to 3+ shots.
                        # Prefix keys still include the exact prior prompts.
                        "width": int(width),
                        "height": int(height),
                        ...
                        "inputs": {
                            ...
                            "keyframe_images": _h3_cache_fingerprint(
                                keyframe_images),
                            "reference_images": _h3_cache_fingerprint(
                                reference_images),
                            "guide_audio": _h3_cache_fingerprint(guide_audio),
                            "voice_ref": _h3_cache_fingerprint(voice_ref),
                            "reference_video": _h3_cache_fingerprint(
                                reference_video),
                            "reference_video_audio": _h3_cache_fingerprint(
                                reference_video_audio),
                        },
                    }
                    _cache_base_key = _h3_cache_key(_base_obj)
                    _root = _h3_cache_root()
                    _cache_dir = os.path.join(_root, _cache_base_key[:16])
                    ...
                    if _cache_mode != "rebuild_cache":
                        for _idx in range(n, 0, -1):
                            _path = _cache_path(_idx)
                            if not os.path.isfile(_path):
                                continue
                            try:
                                try:
                                    _blob = _torch.load(
                                        _path, map_location="cpu",
                                        weights_only=False)
                                except TypeError:
                                    _blob = _torch.load(
                                        _path, map_location="cpu")
                                if (_blob.get("version")
                                        != _H3_SHOT_CACHE_VERSION
                                        or _blob.get("base_key")
                                        != _cache_base_key
                                        or _blob.get("prefix_key")
                                        != _cache_prefix_key(_idx)):
                                    continue
                                _state = _h3_cache_unpack(_blob["state"])
                                bank._entries = list(
                                    _state["bank_entries"])
                                frames_parts = list(_state["frames_parts"])
                                audio_parts = list(_state["audio_parts"])
                                _lat_v_parts = list(_state["lat_v_parts"])
                                _lat_a_parts = list(_state["lat_a_parts"])
                                sr = _state["sr"]
                                voice_block = _state["voice_block"]
                                _cg_ref = _state["cg_ref"]
                                last_tail = _state["last_tail"]
                                _ho_v = _state["ho_v"]
                                _ho_a = _state["ho_a"]
                                _ho_taper_src = _state["ho_taper_src"]
                                _ho_guard = _state["ho_guard"]
                                _ho_wav_tail = _state["ho_wav_tail"]
                                _house_frame = _state["house_frame"]
                                _cc_mu = _state["cc_mu"]
                                _cc_cov = _state["cc_cov"]
                                _cp_prev = _state["cp_prev"]
                                _pin_sig0 = _state["pin_sig0"]
                                _pin_hf0 = _state["pin_hf0"]
                                _cg_last_raw = _state["cg_last_raw"]
                                _cp_trim = _state["cp_trim"]
                                _cache_start = int(_idx)
                                _resume_msg = (
                                    "no sampling needed; assembling cached "
                                    "clips." if _idx >= n else
                                    "rendering starts at clip %d."
                                    % (_idx + 1))
                    ...
                    _cache_enabled = False
                    _cache_write_enabled = False
                    print("[H3Memory] shot_cache disabled: %s"
                          % _e, flush=True)
        ...
        for si, prompt in enumerate(shots[_cache_start:], start=_cache_start):
            ...
            # identity reference image(s): seed identity early, then let the
            # bank carry it afterwards
            ...
            if reference_video is not None and int(reference_video.shape[0]):
                ...
                if si == 1 and _rv_n > 64:
                    print("[H3Memory] reference_video is %d frames; it is "
                          ...
                          "reference tokens on EVERY step. Trim the clip "
                          "before wiring it." % _rv_n, flush=True)
            ...
            if continuity == "flf_chain" and keyframe_images is None:
                ...
                raise _h3_fail(_msg, ValueError, "H3 FLF chain config",
                               tag="H3Memory")
            if continuity == "flf_chain" and keyframe_images is not None:
                ...
                # rejects. Our layout patch generalises the math.
                #
                # KursatAs 2026-08-17 09:54: only an ACTIVE Motion-Context
                # layout patch owns this site. A registered node alone should
                # not block seamless_tail; a live MC layout patch should, since
                # it only serves rows carrying its own marker and these raw
                # keyframes would fall through to stock mid-render.
                try:
                    ...
                    try:
                        from h3_interior_patch import (
                            _motion_context_status, ensure_interior_keyframes)
                    except ImportError:
                        ...
                        _motion_context_status = _m._motion_context_status
                        ensure_interior_keyframes = _m.ensure_interior_keyframes
                _mc_status = _motion_context_status()
                _mc_pack = _mc_status["layout"]
                if _mc_pack:
                    _msg = (
                ...
                if si == 1 and _mc_status["registered"]:
                    ...
                    # node is not enough to block seamless_tail; only an active
                    # layout patch owns the site.
                    _msg = ("seamless_tail: Motion-Context node is registered "
            ...
            if (end_anchor and continuity == "first_frame" and si > 0
                ...
                if si == 1:
                    _msg = ("end_anchor ignored in first_frame mode: a "
                            "last-frame pin makes fl2va plan a camera MOVE "
            ...
            if (continuity == "context_pin" and si > 0
                ...
                print("[H3Memory] context_pin: previous shot's tail pinned "
                      "as raw latents (%sf video + %df audio ref / %.1f s, "
                      "trim %d on decode)"
                      % (_pf, _apf, _apf / 24.0, _cp_trim), flush=True)
            ...
            if (si == 0 and chain_gain_control != "off"
                ...
                if (chain_gain_control == "flatten_pin"
                    ...
                    print("[H3Memory] NOTE: flatten_pin levels decoded frames "
                          "and the bank, and also softens the raw context_pin "
                          "latent before it is pinned. It is experimental and "
                          "not a guaranteed ratchet fix.", flush=True)
                else:
                    print("[H3Memory] NOTE: chain_gain_control corrects the "
                          "decoded frames and the bank, but continuity=%s "
                          "carries the previous shot's RAW LATENTS, which it "
                          "cannot reach - the texture ratchet rides the pin "
                          "regardless (measured +142%% over 3 shots with "
                          "flatten ON). It is effective on frame-carried "
                          "modes (first_frame, cut)." % continuity,
                          flush=True)
            ...
            if continuity == "first_frame" and si > 0 and last_tail is not None:
                ...
                if _m0 >= 0.03:
                    _msg = ("first_frame handover was ignored; frame0 differs "
            ...
            if si > 0 and continuity == "context_pin" and _cp_trim > 0:
                ...
                _blend_prev = (frames_parts[-1] if frames_parts else
                               (_stream_assembler.pending
                                if _stream_assembler is not None else None))
        ...
        if _stream_assembler is not None:
            ...
            _stream_assembler.finalize(_mpath, master_normalize, waveform, sr,
                                    prompt=_api_prompt,
                                    extra_pnginfo=_api_pnginfo)
            _ph = torch.zeros((1, _stream_assembler.shots[0]["h"],
                               _stream_assembler.shots[0]["w"], 3),
                              dtype=torch.half)
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "H3MultiLoader": "H3 Multi Model Loader",
