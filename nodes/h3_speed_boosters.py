...
def _apply(model, class_name, url, label, **kw):
    ...
    def _bypass(reason):
        _msg = ("%s is ON but could not be applied (%s) - rendering at normal "
    ...
    try:
        ...
        return _bypass("incompatible node API: %s" % e)

    # KursatAs 2026-08-17 09:24: installed accelerator packs are still optional;
    # if their INPUT_TYPES/FUNCTION contract changes, bypass instead of killing
    # the H3 render.
    args = {}
    missing = []
    for name, s in spec.items():
        if name == "model":
            ...
            d = s[1] if hasattr(s, "__len__") and len(s) > 1 \
                and isinstance(s[1], dict) else {}
            if "default" in d:
                args[name] = d["default"]
            elif hasattr(s, "__len__") and len(s) > 0 \
                    and isinstance(s[0], list) and s[0]:
                args[name] = s[0][0]
            else:
                missing.append(name)
    if missing:
        return _bypass("required input(s) have no safe default: %s"
                       % ", ".join(missing))
    try:
        out = fn(**args)
    except Exception as e:
        return _bypass("runtime error: %s" % e)
    if isinstance(out, tuple):
        if not out or out[0] is None:
            return _bypass("empty output")
        out = out[0]
    if out is None:
        return _bypass("empty output")
...
class H3SpeedBoosters:
    ...
    DESCRIPTION = ("Optional speed-ups from community packs, behind switches "
                   ...
                   "pack is not installed or no longer matches its expected "
                   "API print an install/update link and do nothing. Measured: "
                   "stacking Spectrum + TeaCache is no faster than Spectrum "
                   "alone - pick one.")

    def boost(self, model, spectrum, teacache, teacache_strength, blockcache,
              easycache=False, blockcache_threshold=0.12):
        if spectrum and teacache:
            _msg = ("Spectrum + TeaCache stacked was measured no faster than "
