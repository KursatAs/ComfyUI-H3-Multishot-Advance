# -*- coding: utf-8 -*-
"""H3 Cartridge Loader - character cartridges for MiniMax-H3.

Reads compatible .riftcast cartridge files (spec v1.0:
...
class H3CartridgeError(ValueError):
    pass
...
def _safe_name(entry_name):
    ...
    if n.startswith("/") or ".." in n.split("/") or (len(n) > 1 and n[1] == ":"):
        raise H3CartridgeError(f"unsafe path in archive: {entry_name!r}")
    return n
...
def _cache_slug(text):
    """Readable, single-directory cache name derived from manifest text."""
    raw = str(text or "").strip()
    slug = "".join(
        ch if (ch.isalnum() or ch in " ._-") else "_" for ch in raw)
    slug = slug.strip(" ._")[:80].strip(" ._")
    return slug or "cartridge"


def _safe_cache_dir(cache_root, manifest_name):
    """Return a cache path guaranteed to stay below cache_root.

    KursatAs 2026-08-17 09:18: Protect the manifest-level character name before
    it becomes the extraction directory/delete target.

    Archive members already go through _safe_name(); this protects the
    manifest-level character name, which becomes the extraction directory and is
    therefore also part of the delete target during re-extraction.
    """
    raw = str(manifest_name or "").strip()
    if not raw:
        raise H3CartridgeError("manifest name must be a non-empty string")

    norm = raw.replace("\\", "/")
    unsafe_path = (norm.startswith("/")
                   or ".." in norm.split("/")
                   or (len(norm) > 1 and norm[1] == ":")
                   or "/" in norm)
    slug = _cache_slug(raw)
    if unsafe_path or slug != raw:
        slug = f"{slug}-{hashlib.sha256(raw.encode('utf-8')).hexdigest()[:12]}"

    root = os.path.abspath(cache_root)
    dst = os.path.abspath(os.path.join(root, slug))
    try:
        inside = os.path.commonpath([root, dst]) == root
    except ValueError:
        inside = False
    if not inside or dst == root:
        raise H3CartridgeError(f"unsafe manifest name for cache: {manifest_name!r}")
    return dst, slug
...
def read_manifest(pack_path):
    with zipfile.ZipFile(pack_path) as z:
        try:
            ...
            raise H3CartridgeError("invalid cartridge: manifest.json missing")
    if "riftcast" not in manifest and "joypack" in manifest:
        manifest["riftcast"] = manifest.pop("joypack")
    for k in SPEC_REQUIRED:
        if k not in manifest:
            raise H3CartridgeError(f"manifest missing required key: {k}")
    return manifest
...
def extract_for_h3(pack_path, cache_root):
    ...
    dst, cache_name = _safe_cache_dir(cache_root, name)
    ...
    if with_stamp != mtime:
        ...
        if os.path.isdir(dst) and os.listdir(dst):
            ...
            stamp = os.path.join(dst, ".pack_mtime")
            print("[H3Cartridge] cartridge cache for %r could not be cleared "
        ...
        with zipfile.ZipFile(pack_path) as z:
            ...
            def ext(member, sub):
                ...
                if member not in names:
                    raise H3CartridgeError(f"manifest references missing file: {member}")
            ...
            if want and _sha256(vpath) != want:
                os.remove(vpath)
                raise H3CartridgeError("voice anchor failed sha256 verification")
...
class H3CartridgeLoader:
    ...
    def load(self, cartridge, max_refs, manual_path=""):
        ...
        if not os.path.isfile(pack):
            raise H3CartridgeError(f"cartridge not found: {pack}")
        ...
        voice = self._load_audio(c["voice"],
                                 os.path.join(c["cache_dir"], "voice"))
